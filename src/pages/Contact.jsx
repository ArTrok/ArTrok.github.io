...
const Contact = () => {
  ...
  function copyEmail () {
    copy('arthurteixeira0805@gmail.com');
    setMessageSent(language === 'portuguese' ? 'Email copiado!' : 'Copied email!');
  }
  ...
  return (
    ...
    <NavBar />
    <motion.div
      variants={skillVariantMain}
      animate="visible"
      initial="hidden"
      className='main_content_container5 gap-5 p-3 justify-items-center bg-deep-purple rounded-md'>

      <motion.h2
        animate={{opacity: 1}}
        initial={{opacity: 0}}
        data-testid="ContactMe"
        className='text-cream font-bold text-4xl'>{language === 'portuguese' ? 'Entre em Contato' : 'Contact Me'}
      </motion.h2>
      ...
      <motion.input
        ...
        animate={{opacity: 1}}
        initial={{opacity: 0}}
        style={{opacity: 1, width: "216px"}}
        type="text" aria-label='Name'
        placeholder={language === 'portuguese' ? 'Nome Completo' : 'Full Name'}
        id='name'
        className='bg-light-blue text-cream h-10 rounded-md p-2 name'/>
      ...
      <motion.input
        ...
        placeholder='E-Mail'
        id="email"
        className='bg-light-blue text-cream rounded-md p-2'/>
        ...
        <motion.textarea
          ...
          rows="4"
          cols="45"
          placeholder={language === 'portuguese' ? 'Mensagem' : 'Message'}
          className='resize-none bg-light-blue text-cream rounded self-stretch p-2' />
      ...
      <motion.button
        ...
        aria-label='Send_It'
        className='text-beige-green bg-light-blue w-40 rounded-md font-bold send'>{language === 'portuguese' ? 'Enviar' : 'Submit'}
      </motion.button>

      <div className='text-cream'>{messageSent}</div>
      
      <motion.p
        className='text-cream emailText'
        onClick={ copyEmail }
        whileHover={{ scale: 1.1}}
        transition={{ type: "spring", stiffness: 500}}
        whileTap={{ scale: 0.9 }}>{language === 'portuguese' ? 'copiar email: arthurteixeira0805@gmail.com' : 'copy email: arthurteixeira0805@gmail.com'}
      </motion.p>
      ...
      <div className="flex gap-5">
        <motion.a
          ...
          transition={{ type: "spring", stiffness: 500}}
          whileTap={{ scale: 0.9, rotate: -1}} href='https://github.com/ArTrok'
          target="_blank"
          rel='noreferrer'
          aria-label='GitHub_L'
        ...
        <motion.a
          ...
          whileHover={{ scale: 1.1, rotate: 1}}
          transition={{ type: "spring", stiffness: 500}}
          whileTap={{ scale: 0.9, rotate: -1}} href='https://www.linkedin.com/in/arthur-teixeira-santos/'
          target="_blank"
          rel='noreferrer'
          aria-label='LinkedIn_L'
          className='flex items-center border-2 border-cream text-cream bg-light-blue shadow-xl p-1 rounded-md link_button'>
        LinkedIn
